...
function getEmulatorNames(sdkPath, binaryOSName, providerName, providerHostName) {
  ...
  cmdExec.get(
    ...
    function(err, data, stderr) {
      ...
      var hostname = ((providerHostName === providerName) ? providerHostName : providerName)
      if (err) {
...
deviceutil.getAVDNames = function(providerName, providerHostName) {
  ...
  if (devutil.androidSDKFileSexists(
    ...
    getEmulatorNames(androidSdkPath, binaryOSName, providerName, providerHostName)
  }
}
...
deviceutil.restartEmulatorByName = function(emuName, port, execArgs) {
  ...
  if (devutil.androidSDKFileSexists(process.env.ANDROID_HOME + '/emulator/' +
      binaryOSName) === true) {
    var androidSdkPath = process.env.ANDROID_HOME + '/emulator/'
